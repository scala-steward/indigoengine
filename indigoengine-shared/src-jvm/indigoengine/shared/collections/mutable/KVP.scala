...
/** Mutable KVP backed by HashMap. Provides in-place mutation operations for performance-sensitive code paths (caching,
  * registers).
  */
final class KVP[A] private (private val _underlying: HashMap[String, A]) extends KVPOps[A, KVP]:
  ...
  def toKVP: ImmutableKVP[A] =
    ImmutableKVP.from(HashMap.from(_underlying))
...
object KVP:
  def empty[A]: KVP[A] =
    new KVP(HashMap.empty[String, A])

  def from[A](kvp: ImmutableKVP[A]): KVP[A] =
    new KVP(HashMap.from(kvp.toHashMap))
